...
const userLogin = asyncHandler(async (req, res) => {
    ...
    if (!user) {
      return res.status(404).json({ message: "User is not registered", success: false });
    }
  
    // If user is not verified
    if (!user.isEmailVarified) {
      await userModel.findOneAndDelete({ email: email });
      return res.status(404).json({ message: "User is not verified", success: false });
    }
    ...
    if (user && await bcrypt.compare(password, user.password)) {
      ...
      return res.status(200).json({message:"User is login sucessfully",data:{
        name: user.name,
        email: user.email,
        role: user.role,
      ...
      // Invalid credentials
      return res.status(401).json({ message: "Either email or password is incorrect", success: false });
    }
  });
